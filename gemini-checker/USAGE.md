...
- **`gemini_checker.sgmodule`** (默认版本)
  - 检测名为 **"Gemini"** 的策略组
  - 推荐配合 `surge_config_example.md` 创建独立Gemini策略组

- **`gemini_checker_usa.sgmodule`** (美国策略组版本)
  - 检测名为 **"美国手动"** 的策略组
  - 适合直接检测现有美国策略组

### 1. 检查配置

**默认版本:** 确保你的Surge配置中有名为 **"Gemini"** 的策略组：
